...
A Component is an element in a simulator that maintains its own state. A typical component also handles events (see [The Event System](event_system.md)) and send/receive requests through ports and connections (see [The Connection System](connection_system.md)). Examples of components can be a CPU core, a cache module, and a DRAM controller.
